...
int read_case()
{
	...
	poly[0][0] = Point(x1, y11);
	poly[0][1] = Point(x1, y2);
	poly[n][2] = Point(x2, y2);
	poly[n][3] = Point(x2, y11);
	scanf("%lf%lf", &Ui, &Li);
	poly[0][2] = Point(Li, y2);
	poly[0][3] = Point(Ui, y11);
	for(int i = 1; i < n; i++)
	{
		poly[i][0] = poly[i-1][3];
		poly[i][1] = poly[i-1][2];
		scanf("%lf%lf", &Ui, &Li);
		poly[i][2] = Point(Li, y2);
		poly[i][3] = Point(Ui, y11);
	}
	poly[n][0] = poly[n-1][3];
	poly[n][1] = poly[n-1][2];
	return 1;
...
void solve()
{
	while(m--)
	{
		...
		for(int i = 0; i <= n; i++)
		{
			if(isPointInPolygon(P, poly[i], 4) != 0) hash[i]++;
		}
	}
